        ...
        {evals.length === 0 ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {evals.map((ev) => {
              ...
              const bandColor = ev.overall_band >= 7 ? '#123F33' : ev.overall_band >= 5.5 ? '#C8A14B' : '#5A1726';
              return (
                <div
                  key={ev.id}
                  style={{
...
function SkillQuad({ avg }: { avg: { ta: number; lr: number; gra: number; cc: number } }) {
  const items = [
    { key: 'ta',  label: 'Task Achievement',    color: '#4A6FA5', val: avg.ta  },
    { key: 'cc',  label: 'Coherence & Cohesion', color: '#3D8567', val: avg.cc  },
    { key: 'lr',  label: 'Lexical Resource',     color: '#9173B8', val: avg.lr  },
    { key: 'gra', label: 'Grammar & Accuracy',   color: '#B5495C', val: avg.gra },
  ];
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
      {items.map((it) => (
        ...
        {evals.length === 0 ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {evals.map((ev) => {
              const isOpen = selected === ev.id;
              const date = new Date(ev.created_at).toLocaleDateString('vi-VN', { day: '2-digit', month: '2-digit', year: 'numeric' });
              const bandColor = ev.overall_band >= 7 ? '#3D8567' : ev.overall_band >= 5.5 ? '#C8A14B' : '#B5495C';
              return (
                <div
                  key={ev.id}
                  style={{
                ...
                >
                  ...
                  >
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
                      <button
                        ...
                        style={{
                          fontFamily: 'var(--font-body)', fontSize: 11,
                          color: copiedId === ev.id ? '#3D8567' : '#9c8657',
                          background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.1)',
                          padding: '5px 12px', borderRadius: 6, cursor: 'pointer', transition: 'all .2s'
                        }}
                      >
                  ...
                  {isOpen && (
                    <div style={{ borderTop: '1px solid rgba(255,255,255,0.06)', padding: '14px 18px', display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 10 }}>
                      {[
                        { label: 'TA', band: ev.ta_band,  color: '#4A6FA5' },
                        { label: 'CC', band: ev.cc_band,  color: '#3D8567' },
                        { label: 'LR', band: ev.lr_band,  color: '#9173B8' },
                        { label: 'GR', band: ev.gra_band, color: '#B5495C' },
                      ].map((c) => (
                        <div key={c.label} style={{ textAlign: 'center' }}>
                          <div style={{ fontFamily: 'var(--font-wordmark)', fontWeight: 900, fontSize: 22, color: c.color }}>{c.band || '—'}</div>
                          <div style={{ fontFamily: 'var(--font-body)', fontSize: 10, color: '#5a4a3a', letterSpacing: '.06em', marginTop: 2 }}>{c.label}</div>
